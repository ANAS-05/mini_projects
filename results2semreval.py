...
headers = {
  ...
  'Referer': 'https://www.osmania.ac.in/res07/20240302.jsp',
...
for roll in range(160422733061,160422733121):
  ...
  if (soup.select('[align="center"][width="50%"]')):
    rollno_result = soup.select('[align="center"][width="50%"]')[-1].get_text().lstrip() #lstrip : remove starting spaces
  elif (soup.select('[width="100%"][align="center"]')[-1].get_text()):
    rollno_result= ' '
  cwd = os.getcwd()
  file_path = os.path.join(cwd,'web_scraper/results2semreval.txt')
